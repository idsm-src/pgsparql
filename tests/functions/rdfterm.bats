load ../psql_tests.bash



####
# isIRI()
#

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_boolean('f'::bool))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_short('0'::int2))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_int('0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_long('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_integer('0'::decimal))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_decimal('0'::decimal))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_float('0'::float4))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_double('0'::float8))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::sparql.zoneddatetime))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::timestamptz, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::sparql.zoneddate))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::date, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_daytimeduration('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_string(''::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_langstring(''::varchar, 'en'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_typedliteral(''::varchar, 'http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_userliteral('123'::int4, 'http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_iri('http://example.org'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int4, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_sblanknode('00000000'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_iri_rdfbox(sparql.rdfbox_create_from_sblanknode(''::varchar, '0'::int4))" {
  expect_output 'f'
}



####
# isBlank()
#

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_boolean('f'::bool))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_short('0'::int2))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_int('0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_long('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_integer('0'::decimal))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_decimal('0'::decimal))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_float('0'::float4))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_double('0'::float8))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::sparql.zoneddatetime))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::timestamptz, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::sparql.zoneddate))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::date, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_daytimeduration('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_string(''::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_langstring(''::varchar, 'en'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_typedliteral(''::varchar, 'http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_userliteral('123'::int4, 'http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_iri('http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int8))" {
  expect_output 't'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int4, '0'::int4))" {
  expect_output 't'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_sblanknode('00000000'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.is_blank_rdfbox(sparql.rdfbox_create_from_sblanknode(''::varchar, '0'::int4))" {
  expect_output 't'
}



####
# isLiteral()
#

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_boolean('f'::bool))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_short('0'::int2))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_int('0'::int4))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_long('0'::int8))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_integer('0'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_decimal('0'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_float('0'::float4))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_double('0'::float8))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::sparql.zoneddatetime))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::timestamptz, '0'::int4))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::sparql.zoneddate))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::date, '0'::int4))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_daytimeduration('0'::int8))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_string(''::varchar))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_langstring(''::varchar, 'en'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_typedliteral(''::varchar, 'http://example.org'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_userliteral('123'::int4, 'http://example.org'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_iri('http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int4, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_sblanknode('00000000'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_sblanknode(''::varchar, '0'::int4))" {
  expect_output 'f'
}



####
# isNumeric()
#

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_boolean('f'::bool))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_short('0'::int2))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_int('0'::int4))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_long('0'::int8))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_integer('0'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_decimal('0'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_float('0'::float4))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_double('0'::float8))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::sparql.zoneddatetime))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::timestamptz, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::sparql.zoneddate))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::date, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_daytimeduration('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_string(''::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_langstring(''::varchar, 'en'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_typedliteral(''::varchar, 'http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_userliteral('123'::int4, 'http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_iri('http://example.org'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int8))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int4, '0'::int4))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_sblanknode('00000000'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_sblanknode(''::varchar, '0'::int4))" {
  expect_output 'f'
}



####
# str()
#

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_boolean('f'::bool))" {
  expect_output 'false'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_boolean('true'::bool))" {
  expect_output 'true'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_short('-32768'::int2))" {
  expect_output '-32768'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_short('32767'::int2))" {
  expect_output '32767'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_int('-2147483648'::int4))" {
  expect_output '-2147483648'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_int('2147483647'::int4))" {
  expect_output '2147483647'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_long('-9223372036854775808'::int8))" {
  expect_output '-9223372036854775808'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_long('9223372036854775807'::int8))" {
  expect_output '9223372036854775807'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_integer('-92233720368547758080'::decimal))" {
  expect_output '-92233720368547758080'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_integer('92233720368547758080'::decimal))" {
  expect_output '92233720368547758080'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_float('-inf'::float4))" {
  expect_output '-INF'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_float('-3.402823466e38'::float4))" {
  expect_output '-3.4028235E38'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_float('-1.175494351e-38'::float4))" {
  expect_output '-1.1754944E-38'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_float('1.175494351e-38'::float4))" {
  expect_output '1.1754944E-38'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_float('3.402823466e38'::float4))" {
  expect_output '3.4028235E38'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_float('inf'::float4))" {
  expect_output 'INF'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_float('nan'::float4))" {
  expect_output 'NaN'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_double('-inf'::float8))" {
  expect_output '-INF'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_double('-1.7976931348623158e308'::float8))" {
  expect_output '-1.7976931348623157E308'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_double('-2.2250738585072014e-308'::float8))" {
  expect_output '-2.2250738585072014E-308'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_double('2.2250738585072014e-308'::float8))" {
  expect_output '2.2250738585072014E-308'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_double('1.7976931348623158e308'::float8))" {
  expect_output '1.7976931348623157E308'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_double('inf'::float8))" {
  expect_output 'INF'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_double('nan'::float8))" {
  expect_output 'NaN'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('0000-02-18T10:00:00Z'::sparql.zoneddatetime))" {
  expect_output '0000-02-18T10:00:00Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('-0010-02-18T20:00:00Z'::sparql.zoneddatetime))" {
  expect_output '-0010-02-18T20:00:00Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('0010-02-18T20:00:00+00:00'::sparql.zoneddatetime))" {
  expect_output '0010-02-18T20:00:00Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('0010-02-18T20:00:00+10:30'::sparql.zoneddatetime))" {
  expect_output '0010-02-18T20:00:00+10:30'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('1990-02-18T00:00:00+14:00'::sparql.zoneddatetime))" {
  expect_output '1990-02-18T00:00:00+14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('2010-02-18T00:00:00-14:00'::sparql.zoneddatetime))" {
  expect_output '2010-02-18T00:00:00-14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('2010-02-18T20:00:00'::sparql.zoneddatetime))" {
  expect_output '2010-02-18T20:00:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('0001-02-18T10:00:00Z BC'::timestamptz, 0::int4))" {
  expect_output '0000-02-18T10:00:00Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('0010-02-18T20:00:00Z BC'::timestamptz, 0::int4))" {
  expect_output '-0009-02-18T20:00:00Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('0010-02-18T20:00:00+00:00'::timestamptz, 0::int4))" {
  expect_output '0010-02-18T20:00:00Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('0010-02-18T20:00:00+10:30'::timestamptz, ((10*60+30)*60)::int4))" {
  expect_output '0010-02-18T20:00:00+10:30'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('1990-02-18T00:00:00+14:00'::timestamptz, (14*60*60)::int4))" {
  expect_output '1990-02-18T00:00:00+14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('2010-02-18T00:00:00-14:00'::timestamptz, (-14*60*60)::int4))" {
  expect_output '2010-02-18T00:00:00-14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_datetime('2010-02-18T20:00:00+00:00'::timestamptz, '-2147483648'::int4))" {
  expect_output '2010-02-18T20:00:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('0000-02-18Z'::sparql.zoneddate))" {
  expect_output '0000-02-18Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('-0010-02-18Z'::sparql.zoneddate))" {
  expect_output '-0010-02-18Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('0010-02-18+00:00'::sparql.zoneddate))" {
  expect_output '0010-02-18Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('0010-02-18+10:30'::sparql.zoneddate))" {
  expect_output '0010-02-18+10:30'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('1990-02-18+14:00'::sparql.zoneddate))" {
  expect_output '1990-02-18+14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('2010-02-18-14:00'::sparql.zoneddate))" {
  expect_output '2010-02-18-14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('2010-02-18'::sparql.zoneddate))" {
  expect_output '2010-02-18'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('0001-02-18 BC'::date, 0::int4))" {
  expect_output '0000-02-18Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('0010-02-18 BC'::date, 0::int4))" {
  expect_output '-0009-02-18Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('0010-02-18'::date, 0::int4))" {
  expect_output '0010-02-18Z'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('0010-02-18'::date, ((10*60+30)*60)::int4))" {
  expect_output '0010-02-18+10:30'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('1990-02-18'::date, (14*60*60)::int4))" {
  expect_output '1990-02-18+14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('2010-02-18'::date, (-14*60*60)::int4))" {
  expect_output '2010-02-18-14:00'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_date('2010-02-18'::date, '-2147483648'::int4))" {
  expect_output '2010-02-18'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_daytimeduration('-9223372036854775808'::int8))" {
  expect_output '-P106751991DT4H54.775808S'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_daytimeduration('-9223372036854775807'::int8))" {
  expect_output '-P106751991DT4H54.775807S'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_daytimeduration('9223372036854775807'::int8))" {
  expect_output 'P106751991DT4H54.775807S'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_string('hello'::varchar))" {
  expect_output 'hello'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_langstring('hello'::varchar, 'en'::varchar))" {
  expect_output 'hello'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_typedliteral('value'::varchar, 'http://example.org'::varchar))" {
  expect_output 'value'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_userliteral('value'::varchar, 'http://example.org'::varchar))" {
  expect_output 'value'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_userliteral('123'::int4, 'http://example.org'::varchar))" {
  expect_output '123'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_userliteral('1.50'::decimal, 'http://example.org'::varchar))" {
  expect_output '1.50'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_userliteral(sparql.ubox_create('123:integer'::sparql.ubox), 'http://example.org'::varchar))" {
  expect_output '123:integer'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_userliteral_with_lexical('123'::int4, 'http://example.org'::varchar, ' 123 '::varchar))" {
  expect_output ' 123 '
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_iri('http://false.org'::varchar))" {
  expect_output 'http://false.org'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int8))" {
  expect_output '(null)'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int4, '0'::int4))" {
  expect_output '(null)'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_sblanknode('00000000'::varchar))" {
  expect_output '(null)'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_sblanknode(''::varchar, '0'::int4))" {
  expect_output '(null)'
}



####
# lang()
#

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_boolean('f'::bool))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_short('0'::int2))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_int('0'::int4))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_long('0'::int8))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_integer('0'::decimal))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_decimal('0'::decimal))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_float('0'::float4))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_double('0'::float8))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::sparql.zoneddatetime))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::timestamptz, '0'::int4))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::sparql.zoneddate))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::date, '0'::int4))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_daytimeduration('0'::int8))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_string(''::varchar))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_langstring(''::varchar, 'en'::varchar))" {
  expect_output 'en'
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_typedliteral(''::varchar, 'http://example.org'::varchar))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_userliteral('123'::int4, 'http://example.org'::varchar))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_iri('http://example.org'::varchar))" {
  expect_output '(null)'
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int8))" {
  expect_output '(null)'
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int4, '0'::int4))" {
  expect_output '(null)'
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_sblanknode('00000000'::varchar))" {
  expect_output '(null)'
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_sblanknode(''::varchar, '0'::int4))" {
  expect_output '(null)'
}



####
# datatype()
#

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_boolean('f'::bool))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#boolean'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_short('0'::int2))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#short'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_int('0'::int4))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#int'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_long('0'::int8))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#long'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_integer('0'::decimal))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#integer'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_decimal('0'::decimal))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#decimal'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_float('0'::float4))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#float'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_double('0'::float8))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#double'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::sparql.zoneddatetime))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#dateTime'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_datetime('0001-01-01T00:00:00Z'::timestamptz, '0'::int4))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#dateTime'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::sparql.zoneddate))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#date'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_date('0001-01-01Z'::date, '0'::int4))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#date'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_daytimeduration('0'::int8))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#dayTimeDuration'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_string(''::varchar))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#string'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_langstring(''::varchar, 'en'::varchar))" {
  expect_output 'http://www.w3.org/1999/02/22-rdf-syntax-ns#langString'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_typedliteral(''::varchar, 'http://example.org'::varchar))" {
  expect_output 'http://example.org'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_userliteral('123'::int4, 'http://example.org'::varchar))" {
  expect_output 'http://example.org'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_iri('http://example.org'::varchar))" {
  expect_output '(null)'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int8))" {
  expect_output '(null)'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_iblanknode('0'::int4, '0'::int4))" {
  expect_output '(null)'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_sblanknode('00000000'::varchar))" {
  expect_output '(null)'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_sblanknode(''::varchar, '0'::int4))" {
  expect_output '(null)'
}



####
# iri()
#

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/./dir'::varchar)" {
  expect_output 'http://example.org/dir'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/../dir'::varchar)" {
  expect_output 'http://example.org/dir'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/./'::varchar)" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/.'::varchar)" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/../'::varchar)" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/..'::varchar)" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/a/./b'::varchar)" {
  expect_output 'http://example.org/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, 'http://example.org/a/../b'::varchar)" {
  expect_output 'http://example.org/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '//example.org/dir?query#tag'::varchar)" {
  expect_output 'https://example.org/dir?query#tag'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '/dir/a/b?query#tag'::varchar)" {
  expect_output 'https://base.org/dir/a/b?query#tag'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '/dir/a/b'::varchar)" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, './dir/a/b'::varchar)" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '../dir/a/b'::varchar)" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir/sub?basequery#basetag'::varchar, './dir/a/b'::varchar)" {
  expect_output 'https://base.org/basedir/dir/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir/sub?basequery#basetag'::varchar, '../dir/a/b'::varchar)" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir/sub/?basequery#basetag'::varchar, './dir/a/b'::varchar)" {
  expect_output 'https://base.org/basedir/sub/dir/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir/sub/?basequery#basetag'::varchar, '../dir/a/b'::varchar)" {
  expect_output 'https://base.org/basedir/dir/a/b'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '?query#tag'::varchar)" {
  expect_output 'https://base.org/basedir?query#tag'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '?query'::varchar)" {
  expect_output 'https://base.org/basedir?query'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '#tag'::varchar)" {
  expect_output 'https://base.org/basedir?basequery#tag'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, ''::varchar)" {
  expect_output 'https://base.org/basedir?basequery'
}

@test "fn: sparql.iri_string('https://base.org/basedir?basequery#basetag'::varchar, '##'::varchar)" {
  expect_output '(null)'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/./dir'::varchar))" {
  expect_output 'http://example.org/dir'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/../dir'::varchar))" {
  expect_output 'http://example.org/dir'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/./'::varchar))" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/.'::varchar))" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/../'::varchar))" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/..'::varchar))" {
  expect_output 'http://example.org/'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/a/./b'::varchar))" {
  expect_output 'http://example.org/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('http://example.org/a/../b'::varchar))" {
  expect_output 'http://example.org/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('//example.org/dir?query#tag'::varchar))" {
  expect_output 'https://example.org/dir?query#tag'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('/dir/a/b?query#tag'::varchar))" {
  expect_output 'https://base.org/dir/a/b?query#tag'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('/dir/a/b'::varchar))" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('./dir/a/b'::varchar))" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('../dir/a/b'::varchar))" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir/sub?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('./dir/a/b'::varchar))" {
  expect_output 'https://base.org/basedir/dir/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir/sub?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('../dir/a/b'::varchar))" {
  expect_output 'https://base.org/dir/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir/sub/?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('./dir/a/b'::varchar))" {
  expect_output 'https://base.org/basedir/sub/dir/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir/sub/?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('../dir/a/b'::varchar))" {
  expect_output 'https://base.org/basedir/dir/a/b'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('?query#tag'::varchar))" {
  expect_output 'https://base.org/basedir?query#tag'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('?query'::varchar))" {
  expect_output 'https://base.org/basedir?query'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('#tag'::varchar))" {
  expect_output 'https://base.org/basedir?basequery#tag'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string(''::varchar))" {
  expect_output 'https://base.org/basedir?basequery'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_string('##'::varchar))" {
  expect_output '(null)'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_iri('http://example.org/dir?query#tag'::varchar))" {
  expect_output 'http://example.org/dir?query#tag'
}

@test "fn: sparql.iri_rdfbox('https://base.org/basedir?basequery#basetag'::varchar, sparql.rdfbox_create_from_langstring('http://example.org/dir?query#tag'::varchar, 'en'))" {
  expect_output '(null)'
}



####
# bnode()
#

@test "fn: sparql.bnode()" {
  expect_output '0'
}



####
# strdt()
#

@test "fn: sparql.strdt_string('true'::varchar, 'http://www.w3.org/2001/XMLSchema#boolean'::varchar)" {
  expect_output '"true"^^<http://www.w3.org/2001/XMLSchema#boolean>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#short'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#short>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#int'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#int>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#long'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#long>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#integer'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#integer>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#decimal'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#decimal>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#float'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#float>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#double'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#double>'
}

@test "fn: sparql.strdt_string(' 0001-01-01T00:00:00Z '::varchar, 'http://www.w3.org/2001/XMLSchema#dateTime'::varchar)" {
  expect_output '" 0001-01-01T00:00:00Z "^^<http://www.w3.org/2001/XMLSchema#dateTime>'
}

@test "fn: sparql.strdt_string(' 0001-01-01Z '::varchar, 'http://www.w3.org/2001/XMLSchema#date'::varchar)" {
  expect_output '" 0001-01-01Z "^^<http://www.w3.org/2001/XMLSchema#date>'
}

@test "fn: sparql.strdt_string(' PT0S '::varchar, 'http://www.w3.org/2001/XMLSchema#dayTimeDuration'::varchar)" {
  expect_output '" PT0S "^^<http://www.w3.org/2001/XMLSchema#dayTimeDuration>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#string'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#string>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#introman'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#introman>'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'www.w3.org/2001/XMLSchema#int'::varchar)" {
  expect_output '(null)'
}



####
# strlang()
#

@test "fn: sparql.strlang_string('value'::varchar, 'en-US'::varchar)" {
  expect_output '"value"@en-us'
}

@test "fn: sparql.strlang_string('value'::varchar, 'en'::varchar)" {
  expect_output '"value"@en'
}

@test "fn: sparql.strlang_string('value'::varchar, 'e'::varchar)" {
  expect_output '(null)'
}

@test "fn: sparql.strlang_string('value'::varchar, ''::varchar)" {
  expect_output '(null)'
}



####
# the integer types derived from xsd:integer
#

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_byte('-128'::int2))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_unsignedbyte('255'::int2))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_unsignedshort('65535'::int4))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_unsignedint('4294967295'::int8))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_unsignedlong('18446744073709551615'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_nonpositiveinteger('-18446744073709551616'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_negativeinteger('-1'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_nonnegativeinteger('0'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.rdfbox_create_from_positiveinteger('18446744073709551616'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_byte('-128'::int2))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_unsignedbyte('255'::int2))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_unsignedshort('65535'::int4))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_unsignedint('4294967295'::int8))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_unsignedlong('18446744073709551615'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_nonpositiveinteger('-18446744073709551616'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_negativeinteger('-1'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_nonnegativeinteger('0'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.is_literal_rdfbox(sparql.rdfbox_create_from_positiveinteger('18446744073709551616'::decimal))" {
  expect_output 't'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_byte('-128'::int2))" {
  expect_output '-128'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_unsignedbyte('255'::int2))" {
  expect_output '255'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_unsignedshort('65535'::int4))" {
  expect_output '65535'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_unsignedint('4294967295'::int8))" {
  expect_output '4294967295'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_unsignedlong('18446744073709551615'::decimal))" {
  expect_output '18446744073709551615'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_nonpositiveinteger('-18446744073709551616'::decimal))" {
  expect_output '-18446744073709551616'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_negativeinteger('-1'::decimal))" {
  expect_output '-1'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_nonnegativeinteger('0'::decimal))" {
  expect_output '0'
}

@test "fn: sparql.str_rdfbox(sparql.rdfbox_create_from_positiveinteger('18446744073709551616'::decimal))" {
  expect_output '18446744073709551616'
}

@test "fn: sparql.str_rdfbox('\"+5\"^^<http://www.w3.org/2001/XMLSchema#unsignedByte>'::sparql.rdfbox)" {
  expect_output '+5'
}

@test "fn: sparql.str_rdfbox('\"-0\"^^<http://www.w3.org/2001/XMLSchema#nonNegativeInteger>'::sparql.rdfbox)" {
  expect_output '-0'
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_byte('-128'::int2))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_unsignedbyte('255'::int2))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_unsignedshort('65535'::int4))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_unsignedint('4294967295'::int8))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_unsignedlong('18446744073709551615'::decimal))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_nonpositiveinteger('-18446744073709551616'::decimal))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_negativeinteger('-1'::decimal))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_nonnegativeinteger('0'::decimal))" {
  expect_output ''
}

@test "fn: sparql.lang_rdfbox(sparql.rdfbox_create_from_positiveinteger('18446744073709551616'::decimal))" {
  expect_output ''
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_byte('-128'::int2))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#byte'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_unsignedbyte('255'::int2))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#unsignedByte'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_unsignedshort('65535'::int4))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#unsignedShort'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_unsignedint('4294967295'::int8))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#unsignedInt'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_unsignedlong('18446744073709551615'::decimal))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#unsignedLong'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_nonpositiveinteger('-18446744073709551616'::decimal))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#nonPositiveInteger'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_negativeinteger('-1'::decimal))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#negativeInteger'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_nonnegativeinteger('0'::decimal))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#nonNegativeInteger'
}

@test "fn: sparql.datatype_rdfbox(sparql.rdfbox_create_from_positiveinteger('18446744073709551616'::decimal))" {
  expect_output 'http://www.w3.org/2001/XMLSchema#positiveInteger'
}

@test "fn: sparql.is_numeric_rdfbox('\"128\"^^<http://www.w3.org/2001/XMLSchema#byte>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"256\"^^<http://www.w3.org/2001/XMLSchema#unsignedByte>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"-1\"^^<http://www.w3.org/2001/XMLSchema#unsignedByte>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"65536\"^^<http://www.w3.org/2001/XMLSchema#unsignedShort>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"4294967296\"^^<http://www.w3.org/2001/XMLSchema#unsignedInt>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"18446744073709551616\"^^<http://www.w3.org/2001/XMLSchema#unsignedLong>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"-1\"^^<http://www.w3.org/2001/XMLSchema#unsignedLong>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"1\"^^<http://www.w3.org/2001/XMLSchema#nonPositiveInteger>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"0\"^^<http://www.w3.org/2001/XMLSchema#negativeInteger>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"-1\"^^<http://www.w3.org/2001/XMLSchema#nonNegativeInteger>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox('\"0\"^^<http://www.w3.org/2001/XMLSchema#positiveInteger>'::sparql.rdfbox)" {
  expect_output 'f'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#byte'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#byte>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#byte'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedByte'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#unsignedByte>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedByte'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedShort'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#unsignedShort>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedShort'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedInt'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#unsignedInt>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedInt'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedLong'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#unsignedLong>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedLong'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' 0 '::varchar, 'http://www.w3.org/2001/XMLSchema#nonPositiveInteger'::varchar)" {
  expect_output '" 0 "^^<http://www.w3.org/2001/XMLSchema#nonPositiveInteger>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 0 '::varchar, 'http://www.w3.org/2001/XMLSchema#nonPositiveInteger'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' -1 '::varchar, 'http://www.w3.org/2001/XMLSchema#negativeInteger'::varchar)" {
  expect_output '" -1 "^^<http://www.w3.org/2001/XMLSchema#negativeInteger>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' -1 '::varchar, 'http://www.w3.org/2001/XMLSchema#negativeInteger'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#nonNegativeInteger'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#nonNegativeInteger>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#nonNegativeInteger'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#positiveInteger'::varchar)" {
  expect_output '" 1 "^^<http://www.w3.org/2001/XMLSchema#positiveInteger>'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string(' 1 '::varchar, 'http://www.w3.org/2001/XMLSchema#positiveInteger'::varchar))" {
  expect_output 't'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string('-1'::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedByte'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string('0'::varchar, 'http://www.w3.org/2001/XMLSchema#positiveInteger'::varchar))" {
  expect_output 'f'
}

@test "fn: sparql.is_numeric_rdfbox(sparql.strdt_string('18446744073709551616'::varchar, 'http://www.w3.org/2001/XMLSchema#unsignedLong'::varchar))" {
  expect_output 'f'
}



####
# str_<type>()
#

@test "fn: sparql.str_byte('-128'::int2)" {
  expect_output '-128'
}

@test "fn: sparql.str_byte('127'::int2)" {
  expect_output '127'
}

@test "fn: sparql.str_unsignedbyte('0'::int2)" {
  expect_output '0'
}

@test "fn: sparql.str_unsignedbyte('255'::int2)" {
  expect_output '255'
}

@test "fn: sparql.str_unsignedshort('65535'::int4)" {
  expect_output '65535'
}

@test "fn: sparql.str_unsignedint('4294967295'::int8)" {
  expect_output '4294967295'
}

@test "fn: sparql.str_unsignedlong('0'::decimal)" {
  expect_output '0'
}

@test "fn: sparql.str_unsignedlong('18446744073709551615'::decimal)" {
  expect_output '18446744073709551615'
}

@test "fn: sparql.str_nonpositiveinteger('-18446744073709551616'::decimal)" {
  expect_output '-18446744073709551616'
}

@test "fn: sparql.str_nonpositiveinteger('0'::decimal)" {
  expect_output '0'
}

@test "fn: sparql.str_negativeinteger('-1'::decimal)" {
  expect_output '-1'
}

@test "fn: sparql.str_nonnegativeinteger('0'::decimal)" {
  expect_output '0'
}

@test "fn: sparql.str_positiveinteger('18446744073709551616'::decimal)" {
  expect_output '18446744073709551616'
}

@test "fn: sparql.str_integer('5'::decimal)" {
  expect_output '5'
}

@test "fn: sparql.str_integer('-5'::decimal)" {
  expect_output '-5'
}

@test "fn: sparql.str_decimal('5'::decimal)" {
  expect_output '5.0'
}

@test "fn: sparql.str_decimal('5.10'::decimal)" {
  expect_output '5.1'
}
