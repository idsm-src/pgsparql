...
PG_FUNCTION_INFO_V1(less_than_rdfbox);
Datum less_than_rdfbox(PG_FUNCTION_ARGS)
{
    ...
    else if(rdfbox_is_numeric(left) && rdfbox_is_numeric(right))
    {
        ...
        else if(left->type == XSD_INTEGER || left->type == XSD_DECIMAL || right->type == XSD_INTEGER || right->type == XSD_DECIMAL)
        {
            ...
            result = DirectFunctionCall2(numeric_lt, NumericGetDatum(l), NumericGetDatum(r));
    ...
    else if(left->type == XSD_DATETIME && right->type == XSD_DATETIME)
    {
        ...
        result = DirectFunctionCall2(zoneddatetime_less_than, ZonedDateTimeGetDatum(l), ZonedDateTimeGetDatum(r));
    }
    else if(left->type == XSD_DATE && right->type == XSD_DATE)
    {
        ZonedDate l = ((RdfBoxDate *) left)->value;
        ZonedDate r = ((RdfBoxDate *) right)->value;
        result = DirectFunctionCall2(zoneddate_less_than, ZonedDateGetDatum(l), ZonedDateGetDatum(r));
    }
