...
PG_FUNCTION_INFO_V1(not_equal_rdfbox);
Datum not_equal_rdfbox(PG_FUNCTION_ARGS)
{
    ...
    else if(rdfbox_is_numeric(left) && rdfbox_is_numeric(right))
    {
        ...
        else if(left->type == XSD_INTEGER || left->type == XSD_DECIMAL || right->type == XSD_INTEGER || right->type == XSD_DECIMAL)
        {
            ...
            result = DirectFunctionCall2(numeric_ne, NumericGetDatum(l), NumericGetDatum(r));
    ...
    else if(left->type == XSD_DATETIME && right->type == XSD_DATETIME)
    {
        ...
        result = DirectFunctionCall2(zoneddatetime_not_equal, ZonedDateTimeGetDatum(l), ZonedDateTimeGetDatum(r));
    }
    else if(left->type == XSD_DATE && right->type == XSD_DATE)
    {
        ZonedDate l = ((RdfBoxDate *) left)->value;
        ZonedDate r = ((RdfBoxDate *) right)->value;
        result = DirectFunctionCall2(zoneddate_not_equal, ZonedDateGetDatum(l), ZonedDateGetDatum(r));
    }
