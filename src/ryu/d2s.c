...
static inline floating_decimal_64 d2d(const uint64_t ieeeMantissa, const uint32_t ieeeExponent, const bool scientific) {
  int32_t e2;
  uint64_t m2;
  if (ieeeExponent == 0) {
  ...
  if (vmIsTrailingZeros || vrIsTrailingZeros) {
    ...
    if (vpDiv100 > vmDiv100 && (vp >= 1000 || !scientific)) { // Optimization: remove two digits at a time (~86.2%).
...
static inline int to_decimal_chars(const floating_decimal_64 v, const bool sign, char* const result) {
  int index = 0;
  if (sign) {
    result[index++] = '-';
  }
  ...
  uint64_t output = v.mantissa;
  const uint32_t olength = decimalLength17(output);
  const int32_t exp = v.exponent + (int32_t) olength - 1;
...
int d2s_buffered_n(double f, char* result, const bool scientific) {
  ...
  if (ieeeExponent == ((1u << DOUBLE_EXPONENT_BITS) - 1u) || (ieeeExponent == 0 && ieeeMantissa == 0)) {
    return copy_special_str(result, ieeeSign, ieeeExponent, ieeeMantissa, scientific);
  }
  ...
  if (isSmallInt) {
    ...
    v = d2d(ieeeMantissa, ieeeExponent, scientific);
  }

  return scientific ? to_chars(v, ieeeSign, result) : to_decimal_chars(v, ieeeSign, result);
}

void d2s_buffered(double f, char* result, const bool scientific) {
  const int index = d2s_buffered_n(f, result, scientific);
...
char* d2s(double f, const bool scientific) {
  ...
  d2s_buffered(f, result, scientific);
  return result;
}
