...
static inline floating_decimal_32 f2d(const uint32_t ieeeMantissa, const uint32_t ieeeExponent, const bool scientific) {
  int32_t e2;
  uint32_t m2;
  if (ieeeExponent == 0) {
...
static inline int to_decimal_chars(const floating_decimal_32 v, const bool sign, char* const result) {
  int index = 0;
  if (sign) {
    result[index++] = '-';
  }
...
int f2s_buffered_n(float f, char* result, const bool scientific) {
  ...
  const floating_decimal_32 v = f2d(ieeeMantissa, ieeeExponent, scientific);
...
void f2s_buffered(float f, char* result, const bool scientific) {
  const int index = f2s_buffered_n(f, result, scientific);
...
char* f2s(float f, const bool scientific) {
  ...
  f2s_buffered(f, result, scientific);
  return result;
}
