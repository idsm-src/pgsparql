...
Datum cast_as_int_from_integer(PG_FUNCTION_ARGS)
{
    ...
    PG_TRY_EX();
    {
        result = DirectFunctionCall1(numeric_int4, NumericGetDatum(value));
    }
    PG_CATCH_EX();
    {
...
Datum cast_as_int_from_decimal(PG_FUNCTION_ARGS)
{
    ...
    PG_TRY_EX();
    {
        result = DirectFunctionCall1(numeric_int4, NumericGetDatum(truncated));
    }
    PG_CATCH_EX();
    {
        if(sqlerrcode != ERRCODE_NUMERIC_VALUE_OUT_OF_RANGE)
...
Datum cast_as_int_from_rdfbox(PG_FUNCTION_ARGS)
{
    ...
    {
        case XSD_BOOLEAN:
            result = DirectFunctionCall1(cast_as_int_from_boolean, BoolGetDatum(((RdfBoxBoolean *) box)->value));
            break;

        case XSD_SHORT:
            result = DirectFunctionCall1(cast_as_int_from_short, Int16GetDatum(((RdfBoxShort *) box)->value));
            break;

        case XSD_INT:
            result = Int32GetDatum(((RdfBoxInt *) box)->value);
            break;

        case XSD_LONG:
            result = DirectFunctionCall1(cast_as_int_from_long, Int64GetDatum(((RdfBoxLong *) box)->value));
            break;

        case XSD_FLOAT:
            result = DirectFunctionCall1(cast_as_int_from_float, Float4GetDatum(((RdfBoxFloat *) box)->value));
            break;

        case XSD_DOUBLE:
            result = DirectFunctionCall1(cast_as_int_from_double, Float8GetDatum(((RdfBoxDouble *) box)->value));
            break;

        case XSD_INTEGER:
            result = DirectFunctionCall1(cast_as_int_from_integer, NumericGetDatum(((RdfBoxInteger *) box)->value));
            break;

        case XSD_DECIMAL:
            result = DirectFunctionCall1(cast_as_int_from_decimal, NumericGetDatum(((RdfBoxDecinal *) box)->value));
            break;

        case XSD_STRING:
            result = DirectFunctionCall1(cast_as_int_from_string, PointerGetDatum(((RdfBoxString *) box)->value));
            break;
