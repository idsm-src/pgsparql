...
Datum cast_as_long_from_rdfbox(PG_FUNCTION_ARGS)
{
    ...
    {
        case XSD_BOOLEAN:
            result = DirectFunctionCall1(cast_as_long_from_boolean, BoolGetDatum(((RdfBoxBoolean *) box)->value));
            break;

        case XSD_SHORT:
            result = DirectFunctionCall1(cast_as_long_from_short, Int16GetDatum(((RdfBoxShort *) box)->value));
            break;

        case XSD_INT:
            result = DirectFunctionCall1(cast_as_long_from_int, Int32GetDatum(((RdfBoxInt *) box)->value));
            break;

        case XSD_LONG:
            PG_RETURN_INT64(((RdfBoxLong *) box)->value);
            break;

        case XSD_FLOAT:
            result = DirectFunctionCall1(cast_as_long_from_float, Float4GetDatum(((RdfBoxFloat *) box)->value));
            break;

        case XSD_DOUBLE:
            result = DirectFunctionCall1(cast_as_long_from_double, Float8GetDatum(((RdfBoxDouble *) box)->value));
            break;

        case XSD_INTEGER:
            result = DirectFunctionCall1(cast_as_long_from_integer, NumericGetDatum(((RdfBoxInteger *) box)->value));
            break;

        case XSD_DECIMAL:
            result = DirectFunctionCall1(cast_as_long_from_decimal, NumericGetDatum(((RdfBoxDecinal *) box)->value));
            break;

        case XSD_STRING:
            result = DirectFunctionCall1(cast_as_long_from_string, PointerGetDatum(((RdfBoxString *) box)->value));
            break;
