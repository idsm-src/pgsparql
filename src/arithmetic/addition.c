...
Datum add_float(PG_FUNCTION_ARGS)
{
...
PG_FUNCTION_INFO_V1(add_double);
Datum add_double(PG_FUNCTION_ARGS)
{
...
PG_FUNCTION_INFO_V1(add_decimal);
Datum add_decimal(PG_FUNCTION_ARGS)
{
    Numeric left = PG_GETARG_NUMERIC(0);
    Numeric right = PG_GETARG_NUMERIC(1);

    Datum result = DirectFunctionCall2(numeric_add, NumericGetDatum(left), NumericGetDatum(right));
...
PG_FUNCTION_INFO_V1(add_rdfbox);
Datum add_rdfbox(PG_FUNCTION_ARGS)
{
